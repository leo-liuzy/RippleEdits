...
if __name__ == '__main__':
    ...
    if editor not in ["no-edit", "in-context"]:
        print("Checkpoint name: ", exp)
    ...
    if model == 'llama3.1-1b-base-eos-sft':
        ...
        query_executor = LookupQueryExecutor(model_name_or_path=f"{os.getenv('PROJ_PLAYGROUND')}/mend/models/Llama-3.2-1B-eos-sft", edit_config_name="llama3.2-1B-eos-sft-mid-upper", use_answer_in_files=offline, ice=editor == "in-context")
    else:
    ...
    if editor == 'mend':
        ...
        model_editor = MENDModelEditorLookup(query_executor, experiment_name=exp)
    elif editor == 'rome':
        ...
        # model_editor = NoEditModelEditor(query_executor)
        model_editor = CPTModelEditorLookup(query_executor, experiment_name=exp)
    elif editor == "know-prop":
        model_editor = KnowledgePropagatorModelEditorLookup(query_executor, experiment_name=exp)
    else:
